...
func validateFile(
	...
	// The organisation match check only applies to the self-onboarding workflow,
	// where a publisher has an explicit alternativeId that the repository
	// declares back in publiccode.yml as `organisation.uri`.
	if publisher.AlternativeID == "" {
		return nil
	}
